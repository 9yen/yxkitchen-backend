...
func (p *Paginator) initProperties(perPage int, baseURL string) {
	...
	p.Order = p.ctx.DefaultQuery(conf.C.Paging.UrlQueryOrder, "asc")
	p.Sort = p.ctx.DefaultQuery(conf.C.Paging.UrlQuerySort, "id")
...
func (p Paginator) getPerPage(perPage int) int {
	// 优先使用请求 per_page 参数
	queryPerpage := p.ctx.Query(conf.C.Paging.UrlQueryPerPage)
...
func (p Paginator) getCurrentPage() int {
	// 优先取用户请求的 page
	page := cast.ToInt(p.ctx.Query(conf.C.Paging.UrlQueryPage))
	if page <= 0 {
		// 默认为 1
		page = 1
	}
...
func (p *Paginator) formatBaseURL(baseURL string) string {
	if strings.Contains(baseURL, "?") {
		baseURL = baseURL + "&" + conf.C.Paging.UrlQueryPage + "="
	} else {
		baseURL = baseURL + "?" + conf.C.Paging.UrlQueryPage+ "="
	}
	return baseURL
...
func (p Paginator) getPageLink(page int) string {
	return fmt.Sprintf("%v%v&%s=%s&%s=%s&%s=%v",
		p.BaseURL,
		page,
		conf.C.Paging.UrlQuerySort,
		p.Sort,
		conf.C.Paging.UrlQueryOrder,
		p.Order,
		conf.C.Paging.UrlQueryPerPage,
		p.PerPage,
	)
}
